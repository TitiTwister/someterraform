resource "outscale_vm" "home_eu_ovpn" {
  image_id           = var.rocky_10_ami
  vm_type            = var.casual_vm_type
  keypair_name       = var.home_dmz_vpn_key
  primary_nic  {
...
resource "outscale_public_ip_link" "ovpn_public_ip_link" {
  vm_id     = outscale_vm.home_eu_ovpn.vm_id
...
resource "outscale_nic" "ovpn_nic" {
    ...
    security_group_ids = [outscale_security_group.sg_dmz.security_group_id]
    private_ips {
      is_primary = true
